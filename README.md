...
The `audit` command writes both the machine-readable comparison JSON and a Markdown report with private-safe structure metrics: DOCX package parts, package flags, body shape, relationship counts, external hyperlink relationship counts, page setup, paragraph property counts, run/style counts, font table names, hyperlinks, tabs, table geometry, numbering definitions, style definitions, XLSX package parts, package part names, relationship summaries, source/generated sheet names, source/generated worksheet paths, dimensions, row counts, column counts, styled-cell counts, headers, source/generated freeze panes, source/generated autofilters, source/generated page margins, source/generated column widths, source/generated cell-type counts, style colors, and cell format details. Audit policy files live under `audit_policies/`; they version accepted drift so review-required differences must be explicitly classified before a release. The default `0.0.2` policy includes one reviewed workbook drift entry for the generated app-native ID/Label columns; all other unmatched differences remain review-required.
